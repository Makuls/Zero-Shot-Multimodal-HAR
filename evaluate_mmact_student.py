...
# ---> IMPORT YOUR EXACT STUDENT MODEL <---
# This imports the TinyStudentNetwork directly from your training script
...
def evaluate_zero_shot():
    print("--- Phase 14: Knowledge Distillation Zero-Shot Evaluation ---")
    ...
    print(f"✅ Successfully located {len(paired_files)} identical trials! Executing forward pass...\n")
    
    print("Loading the Tiny Student Network...")
    
    # Target your specific class name
    model = TinyStudentNetwork(num_classes=27).to(device) 
    
    # Target the specific filename you saved in your distillation script
    model_path = "saved_models/tiny_student_model.pth" 
    
    if not os.path.exists(model_path):
        print(f"❌ [ERROR] Could not find student model at {model_path}.")
        return
    ...
    for i, data_dict in enumerate(paired_files):
        ...
        print(f"[{i+1}/{len(paired_files)}] Analyzing {file_name} (MobileNetV2 + 1D CNN)...")
    ...
    df_results.to_csv("mmact_student_zero_shot_results.csv", index=False)
    print("\n🎉 STUDENT EVALUATION COMPLETE!")
    print("All 542 predictions have been saved to 'mmact_student_zero_shot_results.csv'.")
