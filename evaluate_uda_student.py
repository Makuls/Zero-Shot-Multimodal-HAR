...
# ---> IMPORT THE NEW UDA ARCHITECTURE <---
from train_uda_distillation import UDATinyStudentNetwork
...
def evaluate_uda_zero_shot():
    print("--- Phase 17: UDA Domain-Adapted Zero-Shot Evaluation ---")
    ...
    print("Loading the Hardened UDA Edge Model...")
    model = UDATinyStudentNetwork(num_classes=27).to(device)
    model.load_state_dict(torch.load("saved_models/uda_student_model.pth", map_location=device))
    model.eval()
    ...
    for i, data_dict in enumerate(paired_files):
        ...
        with torch.no_grad():
            # Alpha is None, so the model runs in pure inference mode (ignores the discriminator)
    ...
    df_results.to_csv("mmact_uda_results.csv", index=False)
    print("\n🎉 UDA EVALUATION COMPLETE!")
    print("Results saved to 'mmact_uda_results.csv'.")

if __name__ == "__main__":
    evaluate_uda_zero_shot()